...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('konten.update', $konten->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-6">
            @csrf
            @method('PUT')

            <div class="mb-6">
                <label for="judul" class="block text-sm font-medium text-gray-700 mb-2">Judul Artikel</label>
                <input type="text" name="judul" id="judul" value="{{ old('judul', $konten->judul) }}"
            ...
            <div class="mb-6">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="6"
                          ...
                          required>{{ old('deskripsi', $konten->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
